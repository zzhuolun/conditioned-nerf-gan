"""
Code from the Convolutional Occupancy Network:
    ...
    def forward(self, x):
        ...
        if self.return_global:
            global_features = torch.flatten(
                nn.AvgPool3d(kernel_size=x.shape[-1])(x), start_dim=1
            )
...
class Abstract3DPyramidUNet(nn.Module):
    """
    Base class for standard and residual UNet.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(Abstract3DPyramidUNet, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.return_global:
            return feature_pyramid, global_features
        else:
...
class PyramidUNet3D(Abstract3DPyramidUNet):
    """
    3DUnet model from
    `"3D U-Net: Learning Dense Volumetric Segmentation from Sparse Annotation"
        <https://arxiv.org/pdf/1606.06650.pdf>`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(PyramidUNet3D, self).__init__(
            in_channels=in_channels,
            out_channels=out_channels,
            final_sigmoid=final_sigmoid,
            basic_module=DoubleConv,
