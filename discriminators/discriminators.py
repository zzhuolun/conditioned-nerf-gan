...
class AdapterBlock(nn.Module):
    def __init__(self, output_channels, input_channels=3):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, 1, padding=0), nn.LeakyReLU(0.2)
        )
...
class ResidualCoordConvBlock(nn.Module):
    ...
    def forward(self, identity):
        ...
        y = (y + identity) / math.sqrt(2)  # add residue here
        return y
...
class ProgressiveDiscriminator(nn.Module):
    ...
    def forward(self, input, alpha, instance_noise=0, cond=None, **kwargs):
        start = self.img_size_to_layer[
            input.shape[-1]
        ]  # During training, progressively increase generated image resolution and add new layers to the discriminator
...
class ProgressiveDiscriminator_inputCat(nn.Module):
    """Implement of a progressive growing discriminator with ResidualCoordConv Blocks"""
...
class ProgressiveDiscriminator_projection(nn.Module):
    """Implement of a progressive growing discriminator with ResidualCoordConv Blocks"""
    ...
    def __init__(self, **kwargs):
        ...
        self.mapping = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=False)
        self.mapping.fc = nn.Linear(512, 1600, bias=True)
    ...
    def forward(self, input, alpha, instance_noise=0, cond=None, **kwargs):
        ...
        # conditioning
        cmap = self.mapping(cond)
        cmap = nn.functional.normalize(cmap)
